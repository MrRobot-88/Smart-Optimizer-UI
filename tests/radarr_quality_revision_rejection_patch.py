...
if len(sys.argv) != 2:
    raise SystemExit("usage: radarr_quality_revision_rejection_patch.py UI")
...
new_classifier = '''        if (
            "not an upgrade for existing movie file" in lower_message
            or "not a quality revision upgrade for existing movie file" in lower_message
        ):
            health = "Optimizer import blocked"
            health_kind = "optimizer_blocked"
'''
...
if ui.count(old_classifier) != 1:
    raise SystemExit(
        "STOP: queue-health classifier anchor count=%d"
...
new_allowed = '''"not an upgrade for existing movie file",
...
if count != 3:
    raise SystemExit(
        "STOP: expected 3 allowed-rejection anchors in patched UI, found %d"
        % count
    )
...
print("RADARR QUALITY-REVISION REJECTION PATCH COMPLETE")
print(
    "UI SHA256",
